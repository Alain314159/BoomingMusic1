...
class CoverProvider : ContentProvider(), KoinComponent {
    ...
    override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
        ...
        // If cache not available, schedule async generation and return fallback immediately
        try {
            val ctx = context
            if (ctx != null) {
                GlobalScope.launch(Dispatchers.IO) {
                    try {
                        ...
                        if (data != null) {
                            ...
                            if (bitmap != null) {
                                // write to a temp file then move to cacheFile to avoid race conditions
                                try {
                                    val tmp = File.createTempFile("${matchCode}_${id}_", ".jpg", cacheDir)
                                    var success = false
                                    tmp.outputStream().use {
                                        success = bitmap.compress(Bitmap.CompressFormat.JPEG, 100, it)
                                    }
                                    if (success) {
                                    ...
                                    Log.d(TAG, "Failed to write cache file", e)
                                }
                            }
                        }
                    } catch (e: Exception) {
                        Log.d(TAG, "Async cover generation failed", e)
                    }
                }
            }
        } catch (e: Exception) {
            Log.d(TAG, "Scheduling async cover generation failed", e)
        }
    ...
    override fun getType(uri: Uri): String {
        if (uriMatcher.match(uri) == UriMatcher.NO_MATCH) throw IllegalArgumentException("Unknown URI: $uri")
        return "image/*"
    }
